...
async function createTauntScreen(options: TauntOptions) {
  ...
  try {
    // Load fighter images
    const winnerImage = await loadImage(options.winnerImage);
    const loserImage = await loadImage(options.loserImage);
    ...
    // Add winner and loser labels
    ctx.font = "20px SF2Font";
    ctx.fillStyle = "#FFFFFF";
    ctx.textAlign = "center";
    ctx.fillText(
      options.winnerName,
      padding + imageWidth / 2,
      padding + imageHeight + 30,
    );
    ctx.fillText(
      options.loserName,
      canvasWidth - padding - imageWidth / 2,
      padding + imageHeight + 30,
    );

    // Add taunt text below images
...
function parseArgs(): TauntOptions | null {
  const args = process.argv.slice(2);

  if (args.length < 4) {
    console.log(
      "Usage: ts-node sf2-taunt.ts <winnerName> <winnerImagePath> <loserName> <loserImagePath>",
    );
    console.log(
      "Example: ts-node sf2-taunt.ts 'Caity' './assets/caity.jpg' 'Paul' './assets/paul.jpg'",
    );
    return null;
  }
